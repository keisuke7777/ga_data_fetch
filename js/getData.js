  ...
  let id = setInterval(() => {
    ...
    if (baseInfo.length > 0) {
      ...
      if (customerId[0] === userId) {
        const dates = new Set(
          [...elem.contentWindow.document.getElementsByClassName("_GAOyb")].map(
            (e) => e.innerHTML
          )
        );
...
const getSessions = (SessionPerDates) => {
  ...
  const sessions = SessionPerDates.map((elem) => {
    ...
    return {
      ...
      // セッション別
      sessions: [...elem.getElementsByClassName("_GAGf")].map((sessionElem) => {
        return {
          sessionTime: sessionElem.getElementsByClassName("_GAMI")[0].innerHTML,
          duration: sessionElem.getElementsByClassName("_GARE")[0].innerHTML,
          channel: sessionElem.getElementsByClassName("_GArR")[0].innerHTML,
...
let reports = [];
const getDate = () => {
  chrome.runtime.sendMessage({ method: "getItem", key: "res" }, (response) => {
    if (response.data) {
      userList = response.data;
      console.log("userList.length", userList.length);
      userList = userList.slice(500, 1000);
